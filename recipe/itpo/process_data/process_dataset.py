...
def collapse_example(example: dict[str, Any]) -> dict[str, Any]:
    ...
    assert (
        "single_turn_prompt" in extra_info
    ), "Missing 'single_turn_prompt' in extra_info."

    # add system prompt as the beginning of the list
    example["prompt"] = [{"role": "system", "content": SYSTEM_PROMPT}] + example[
        "prompt"
    ][:1]
    ...
    return {
        ...
        "agent_name": "itpo_collabllm_agent",
...
def main():
    ...
    ap.add_argument(
        "--dataset",
        default="collabllm/collabllm-multiturn-math-hard",
        help="HF dataset path or local dir/file.",
    )
    ap.add_argument(
        "--task_desc",
        default="solving math problems",
        help="Task description for the dataset.",
    )
    ap.add_argument(
        "--local_dir", default="~/data/collabllm-math-hard", help="Output directory."
    )
    ap.add_argument(
        "--hdfs_dir", default=None, help="Optional HDFS destination (requires verl)."
    )
    ap.add_argument(
        "--validation_size",
        type=float,
    ...
    ap.add_argument(
        "--num_proc", type=int, default=1, help="Parallel workers for map()."
    )
    ap.add_argument(
        "--dataset_type",
        default="rl",
    ...
    if args.dataset_type == "rl":
        ...
        ds_all = ds_all.map(
            lambda x: {"task_desc": args.task_desc}, num_proc=args.num_proc
        )
        ...
        def dedup_by_prompt(dataset):
            ...
            for ex in dataset:
                prompt_key = json.dumps(
                    ex["prompt"], sort_keys=True, ensure_ascii=False
                )
        ...
        df = df.sort_values(
            ["conv_id", "turn_id", "score"], ascending=[True, False, False]
        )
    ...
    print(
        f"[INFO] Splitting with validation_size={args.validation_size}, seed={args.seed}"
    )
    ...
    print(
        f"[DONE] {args.dataset_type}_train.parquet and {args.dataset_type}_validation.parquet written."
    )
