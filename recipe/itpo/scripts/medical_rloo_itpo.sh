...
TASK="medical"
ROOT_FOLDER=''
collab_train_path="$ROOT_FOLDER/data/${TASK}/rl_train.parquet"
collab_val_path="$ROOT_FOLDER/data/${TASK}/rl_validation.parquet"
...
python3 -m recipe.itpo.main_prime \
    ...
    reward_model.model.update=itpo \
    reward_model.model.beta_train=0.0035 \
    ...
    trainer.experiment_name=$TASK-rloo-itpo \
    trainer.n_gpus_per_node=2 \
    trainer.nnodes=1 \
    trainer.save_freq=50 \
    trainer.test_freq=1000 \
    trainer.total_epochs=20 $@ \
    reward_model.reward_manager=medical \
    +reward_model.reward_kwargs.metric_weights.diagnosis2=1 \
    ...
    actor_rollout_ref.rollout.multi_turn.interaction_config_path="$ROOT_FOLDER/verl/recipe/itpo/config/itpo_interaction_medical.yaml" \
    trainer.resume_mode=disable \
    trainer.resume_from_path=$RESUME_PATH \
    reward_model.enable_resource_pool=True \
    >./${TASK}_rloo_itpo.log 2>&1 & </dev/null &
